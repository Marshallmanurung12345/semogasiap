...
self.addEventListener('push', (event) => {
  ...
  const options = {
    body: data.message || data.body || 'Ada cerita baru dari komunitas.',
    icon: data.icon || './favicon.png',
    badge: data.badge || './favicon.png',
    data: { url: data.url || (data.storyId ? `#/story/${data.storyId}` : '#/home') },
    tag: data.tag || 'cerita-kita-story',
    renotify: true,
    actions: data.storyId ? [{ action: 'open-story', title: 'Lihat cerita' }] : []
  };
