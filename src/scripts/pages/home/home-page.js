...
import { getCachedStories, getSavedStories, saveStory, deleteSavedStory, saveStories } from '../../data/db';
...
const HomePage = {
  ...
  async afterRender() {
    ...
    try {
      ...
      document.querySelector('#story-count').textContent = `${stories.length} cerita`;
    } catch (error) {
      stories = await getCachedStories();
      document.querySelector('#story-count').textContent = `${stories.length} cerita (offline)`;
      setMessage('Koneksi tidak tersedia. Menampilkan data terakhir yang tersimpan.', 'error');
    }
    ...
    renderCards(savedList, savedStories, new Set(savedStories.map((story) => story.id)));

    const map = document.querySelector('#story-map');
    map.setStories(stories);
    document.querySelector('#fit-map').addEventListener('click', () => {
      const withLocation = stories.filter((s) => Number.isFinite(s.lat) && Number.isFinite(s.lon));
      if (withLocation.length) map.map.fitBounds(L.latLngBounds(withLocation.map((s) => [s.lat, s.lon])), { padding: [30, 30] });
    });
    ...
    const handleSaveAction = async (event) => {
      ...
      if (event.target.closest('.save-story')) {
        ...
        if (currentlySaved) await deleteSavedStory(id); else await saveStory(story);
        savedStories = await getSavedStories();
        const ids = new Set(savedStories.map((item) => item.id));
        renderCards(list, stories, ids);
        renderCards(savedList, savedStories, ids);
        setMessage(currentlySaved ? 'Cerita dihapus dari penyimpanan offline.' : 'Cerita disimpan untuk dibaca offline.', 'success');
      }
    ...
    document.querySelector('#refresh-saved').addEventListener('click', async () => {
      savedStories = await getSavedStories();
      renderCards(savedList, savedStories, new Set(savedStories.map((item) => item.id)));
    });
  },
};
