...
export const getVapidPublicKey = async () => {
  const response = await fetch(`${CONFIG.BASE_URL}/notifications/vapid-public-key`, {
    headers: authHeaders(),
  });
...
export const subscribePush = async () => {
  ...
  if (!subscription) {
    ...
    if (!publicKey) throw new Error('VAPID public key tidak tersedia dari API.');
  ...
  if (!response.ok) throw new Error('Langganan push notification gagal disimpan.');
  return subscription;
};
...
export const unsubscribePush = async () => {
  ...
  if (!response.ok) throw new Error('Langganan push notification gagal dihentikan.');
